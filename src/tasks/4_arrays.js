...
function isExpressionValid(expression) {
  ...
  for (let i = firstClosingPosition; i < expression.length; i++) {
    if (expression[i] === "}" && bracesStack[bracesStack.length - 1] === "{")
      bracesStack.pop();
    else if (
      expression[i] === ")" &&
      bracesStack[bracesStack.length - 1] === "("
    )
      bracesStack.pop();
    else if (
      expression[i] === "]" &&
...
function getEligibleUserNames(users) {
  return users.filter(({ age }) => age >= 18).map(user => user.name);
}
...
function calculateCart(items) {
  return items.reduce(
    (accum, currentItem) => (accum += currentItem.price * currentItem.amount),
    0
  );
}
